...
class Myorder extends MY_Controller
{
    ...
    public function index($page = null)
    {
        ...
        $data['content']    = $this->myorder->where('id_user', $this->id)->paginate($page)->get();  // Mengambil data-data dalam bentuk objek
        $data['total_rows'] = $this->myorder->count();
        $data['pagination'] = $this->myorder->makePagination(          // Generate pagination link
            base_url('myorder'),
    ...
    public function confirm($invoice)
    {
        ...
        if ($this->myorder->create($data['input'])) {   // Jika insert berhasil
            ...
            $this->myorder->table = 'order_detail';
            // var_dump($this->myorder->where('id_orders', $param)->get());
            $param = $data['input']->id_orders;
            $item = $this->myorder->where('id_orders', $param)->get();
            foreach ($item as $get_item) {
                $this->myorder->table = 'product';
                $getItem = $this->myorder->where('id', $get_item->id_product)->first();
                $oldstock = $getItem->stock;
                $newstock = $oldstock - $get_item->qty;
                $this->myorder->where('id', $get_item->id_product)->update(['stock' => $newstock]);
            }
