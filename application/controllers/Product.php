...
class Product extends MY_Controller
{
    ...
    public function index($page = null)
    {
        ...
        $data['content']    = $this->product->select(
            [
                'product.id', 'product.title AS product_title', 'product.image', 'product.price', 'product.stock',
    ...
    {
        $data['content'] = $this->product->where('id', $id)->first();
        $getdata = $this->product->where('id', $id)->first();
        ...
        if (!$_POST) {
            ...
            $data['input'] = (object) $this->input->post(null, true);
            $tmp = intval($data['input']->stock) + $getdata->stock;
            $data['input']->stock =  $tmp;
        }

        if (!$this->product->validate()) {
            $data['title']          = 'Ubah Produk';
            $data['form_action']    = base_url("product/restock/$id");
            $data['page']           = 'pages/product/restock';
