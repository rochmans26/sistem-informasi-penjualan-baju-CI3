...
class Cart extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $data['content']    = $this->cart->select([
            'cart.id', 'cart.qty', 'cart.size', 'cart.subtotal',
    ...
    public function add()
    {
        if (!$_POST || $this->input->post('qty') < 1) {
            ...
            // if ($produk->stock == 0) {   // Jika insert berhasil
            //     $this->session->set_flashdata('error', 'Oops! Stock Produk Barang Kosong!');
            // } else {
            //     $this->cart->create($data);
            //     $this->session->set_flashdata('success', 'Produk berhasil ditambahkan');
            // }

            // var_dump((int)$product->stock);
            if ((int)$data['qty'] > (int)$product->stock) {   // Jika insert berhasil
                $this->session->set_flashdata('error', 'Oops! Ketersediaan Stock Tidak Memadai!');
            } else {
                $this->cart->create($data);
                $this->session->set_flashdata('success', 'Produk berhasil ditambahkan');
            }
